...
for i in tqdm(range(1,13,1)):
    if len(assume_id) ==12:
        ...
        if i < 10:
            index = "0" +  str(i)
            race_id = assume_id + index
        else:
            index = i
            race_id = assume_id + str(index)
    ...
    is_denso = True
    if is_denso:
        save_path = "/home/denso/horse_inference/infrecence/" + str(assume_id) + "/"
    else:
        save_path = "/home/hayato/horse_inference/infrecence/" + str(assume_id) + "/"
    # 保存先のディレクトリが存在しない場合は作成
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    # レース結果のスクレイピング
    results = Results.scrape(race_id_list)
    results.to_csv(save_path + str(assume_id)+ str(i) + ".csv", index=False)