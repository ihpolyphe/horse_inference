...
for n in tqdm.tqdm((range(len(pastIndex)))):#nはレース数に対応
    ...
    xNList,yNList,horse_name = learning_method.makeExplainList_as_horse(n,var_addNum, pastIndex, data, need, decreaseRate, otherNeed, xListBefore, yListBefore)
...
for i in range(len(predict)):
    list = [horse_name_list[i],predict[i],yTest[i]]
    save_list.append(list)

column=["horse_name", "inference_horse_time", "正解のレースタイム"]
df = pd.DataFrame(save_list, columns=column)
df.to_csv("/mnt/c/Users/hayat/Desktop/keiba_analysis/inference/inference_race_time" + now.strftime('%Y_%m_%d_%H_%M_%S') +".csv")