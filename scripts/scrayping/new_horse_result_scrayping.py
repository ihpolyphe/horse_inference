...
is_save = False
if is_save:
    results = pd.read_csv(save_path+"train_data_results_" + str(scrayping_year) + ".csv")

else:
    # レース結果のスクレイピング
    print("start horse race scrayping")
    results = Results.scrape(race_id_list)
    results.to_csv(save_path+"train_data_results_" + str(scrayping_year) + ".csv", index=False)


# 馬の過去成績のスクレイピング
horse_id_list = results['horse_id'].unique()
jockey_id_list = results['jockey_id'].unique()
print("start horse result race scrayping")
...
# 馬の血統データのスクレイピング
print("start Peds scrayping")
horse_scrape_results = Peds.scrape(horse_id_list)
